...
import (
	...
	"encoding/binary"
	"fmt"
	counter2 "github.com/noah-blockchain/noah-node/abci/example/counter"
	kvstore2 "github.com/noah-blockchain/noah-node/abci/example/kvstore"
	server2 "github.com/noah-blockchain/noah-node/abci/server"
...
func checkTxs(t *testing.T, mempool Mempool, count int, peerID uint16) types.Txs {
	...
	for i := 0; i < count; i++ {
		...
		if err := CheckTxWithInfo(txBytes, nil, txInfo); err != nil {
			// Skip invalid txs.
...
func TestReapMaxBytesMaxGas(t *testing.T) {
	...
	require.Equal(t, app.CheckTx(types2.RequestCheckTx{Tx: tx0.tx}).GasWanted, int64(1), "KVStore had a gas value neq to 1")
...
func TestMempoolFilters(t *testing.T) {
	...
	nopPostFilter := func(tx types.Tx, res *types2.ResponseCheckTx) error { return nil }
	...
	for tcIndex, tt := range tests {
		mempool.Update(1, emptyTxArr, abciResponses(len(emptyTxArr), types2.CodeTypeOK), tt.preFilter, tt.postFilter)
		checkTxs(t, mempool, tt.numTxsToCreate, UnknownPeerID)
...
func TestMempoolUpdate(t *testing.T) {
	...
	{
		mempool.Update(1, []types.Tx{[]byte{0x01}}, abciResponses(1, types2.CodeTypeOK), nil, nil)
	...
	// 2. Removes valid txs from the mempool
	{
		...
		mempool.Update(1, []types.Tx{[]byte{0x02}}, abciResponses(1, types2.CodeTypeOK), nil, nil)
...
func TestTxsAvailable(t *testing.T) {
	...
	if err := mempool.Update(1, committedTxs, abciResponses(len(committedTxs), types2.CodeTypeOK), nil, nil); err != nil {
		t.Error(err)
	}
	ensureFire(t, mempool.TxsAvailable(), timeoutMS)
	ensureNoFire(t, mempool.TxsAvailable(), timeoutMS)
	...
	if err := mempool.Update(2, committedTxs, abciResponses(len(committedTxs), types2.CodeTypeOK), nil, nil); err != nil {
		t.Error(err)
	}
	ensureNoFire(t, mempool.TxsAvailable(), timeoutMS)
...
func TestSerialReap(t *testing.T) {
	app := counter2.NewCounterApplication(true)
	app.SetOption(types2.RequestSetOption{Key: "serial", Value: "on"})
	cc := proxy.NewLocalClientCreator(app)
	...
	updateRange := func(start, end int) {
		...
		if err := mempool.Update(0, txs, abciResponses(len(txs), types2.CodeTypeOK), nil, nil); err != nil {
			t.Error(err)
		}
	}
	...
	commitRange := func(start, end int) {
		...
		for i := start; i < end; i++ {
			txBytes := make([]byte, 8)
			binary.BigEndian.PutUint64(txBytes, uint64(i))
			res, err := appConnCon.DeliverTxSync(types2.RequestDeliverTx{Tx: txBytes})
			if err != nil {
...
func TestMempoolTxsBytes(t *testing.T) {
	...
	app2 := counter2.NewCounterApplication(true)
	...
	mempool.Update(1, []types.Tx{}, abciResponses(0, types2.CodeTypeOK), nil, nil)
	assert.EqualValues(t, 0, mempool.TxsBytes())
}
...
func newRemoteApp(t *testing.T, addr string, app types2.Application) (clientCreator proxy.ClientCreator, server cmn.Service) {
	clientCreator = proxy.NewRemoteClientCreator(addr, "socket", true)

	// Start server
	server = server2.NewSocketServer(addr, app)
...
func abciResponses(n int, code uint32) []*types2.ResponseDeliverTx {
	responses := make([]*types2.ResponseDeliverTx, 0, n)
	for i := 0; i < n; i++ {
		responses = append(responses, &types2.ResponseDeliverTx{Code: code})
	}
	return responses
}
