...
func (tr *TestReactor) Receive(chID byte, peer Peer, msgBytes []byte) {
	if tr.logMessages {
		...
		tr.msgsReceived[chID] = append(tr.msgsReceived[chID], PeerMessage{ID(), msgBytes, tr.msgsCounter})
		tr.msgsCounter++
	}
}
...
func TestSwitches(t *testing.T) {
	...
	if Size() != 1 {
		t.Errorf("Expected exactly 1 peer in s1, got %v", Size())
	}
	if Size() != 1 {
		t.Errorf("Expected exactly 1 peer in s2, got %v", Size())
	}
...
func TestSwitchFiltersOutItself(t *testing.T) {
	...
	assert.True(t, OurAddress(rp.Addr()))
	assert.False(t, HasAddress(rp.Addr()))
...
func assertNoPeersAfterTimeout(t *testing.T, sw *Switch, timeout time.Duration) {
	time.Sleep(timeout)
	if Size() != 0 {
		t.Fatalf("Expected %v to not connect to some peers, got %d", sw, Size())
	}
}
...
func TestSwitchStopPeerForError(t *testing.T) {
	...
	p := List()[0]
	Send(0x1, []byte("here's a message to send"))
...
func TestSwitchReconnectsToOutboundPersistentPeer(t *testing.T) {
	...
	assert.Equal(t, 1, Size())     // new peer instance
...
func TestSwitchFullConnectivity(t *testing.T) {
	...
	for i, sw := range switches {
		if Size() != 2 {
			t.Fatalf("Expected each switch to be connected to 2 other, but %d switch only connected to %d", Size(), i)
		}
	}
}
...
func TestSwitchAcceptRoutine(t *testing.T) {
	...
	time.Sleep(10 * time.Millisecond)
	assert.Equal(t, cfg.MaxNumInboundPeers, Size())
...
func TestSwitchInitPeerIsNotCalledBeforeRemovePeer(t *testing.T) {
	...
	go sw.StopPeerForError(Get(rp.ID()), "test")
