...
type AddrBook interface {
	...
	AddOurAddress(*p2p2.NetAddress)
	// Check if it is our address
	OurAddress(*p2p2.NetAddress) bool

	AddPrivateIDs([]string)

	// Add and remove an address
	AddAddress(addr *p2p2.NetAddress, src *p2p2.NetAddress) error
	RemoveAddress(*p2p2.NetAddress)

	// Check if the address is in the book
	HasAddress(*p2p2.NetAddress) bool
	...
	PickAddress(biasTowardsNewAddrs int) *p2p2.NetAddress

	// Mark address
	MarkGood(p2p2.ID)
	MarkAttempt(*p2p2.NetAddress)
	MarkBad(*p2p2.NetAddress)

	IsGood(*p2p2.NetAddress) bool

	// Send a selection of addresses to peers
	GetSelection() []*p2p2.NetAddress
	// Send a selection of addresses with bias
	GetSelectionWithBias(biasTowardsNewAddrs int) []*p2p2.NetAddress
...
type addrBook struct {
	...
	privateIDs map[p2p2.ID]struct{}
	addrLookup map[p2p2.ID]*knownAddress // new & old
...
func NewAddrBook(filePath string, routabilityStrict bool) *addrBook {
	am := &addrBook{
		...
		privateIDs:        make(map[p2p2.ID]struct{}),
		addrLookup:        make(map[p2p2.ID]*knownAddress),
		filePath:          filePath,
...
func (a *addrBook) AddOurAddress(addr *p2p2.NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) OurAddress(addr *p2p2.NetAddress) bool {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) AddPrivateIDs(ids []string) {
	...
	for _, id := range ids {
		a.privateIDs[p2p2.ID(id)] = struct{}{}
	}
}
...
func (a *addrBook) AddAddress(addr *p2p2.NetAddress, src *p2p2.NetAddress) error {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) RemoveAddress(addr *p2p2.NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) IsGood(addr *p2p2.NetAddress) bool {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) HasAddress(addr *p2p2.NetAddress) bool {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) PickAddress(biasTowardsNewAddrs int) *p2p2.NetAddress {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) MarkGood(id p2p2.ID) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) MarkAttempt(addr *p2p2.NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) MarkBad(addr *p2p2.NetAddress) {
...
func (a *addrBook) GetSelection() []*p2p2.NetAddress {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	...
	allAddr := make([]*p2p2.NetAddress, bookSize)
	i := 0
...
// bucket.
func (a *addrBook) GetSelectionWithBias(biasTowardsNewAddrs int) []*p2p2.NetAddress {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *addrBook) addAddress(addr, src *p2p2.NetAddress) error {
	if addr == nil || src == nil {
...
func (a *addrBook) randomPickAddresses(bucketType byte, num int) []*p2p2.NetAddress {
	...
	selection := make([]*p2p2.NetAddress, 0, num)
...
func (a *addrBook) calcNewBucket(addr, src *p2p2.NetAddress) int {
	data1 := []byte{}
...
func (a *addrBook) calcOldBucket(addr *p2p2.NetAddress) int {
	data1 := []byte{}
	data1 = append(data1, []byte(a.key)...)
...
// address.
func (a *addrBook) groupKey(na *p2p2.NetAddress) string {
