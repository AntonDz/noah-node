...
type PEXReactor struct {
	...
	seedAddrs []*p2p2.NetAddress

	attemptsToDial sync.Map // address (string) -> {number of attempts (int), last time dialed (time.Time)}

	// seed/crawled mode fields
	crawlPeerInfos map[p2p2.ID]crawlPeerInfo
}
...
func NewPEXReactor(b AddrBook, config *PEXReactorConfig) *PEXReactor {
	r := &PEXReactor{
		...
		crawlPeerInfos:       make(map[p2p2.ID]crawlPeerInfo),
	}
	r.BaseReactor = *p2p2.NewBaseReactor("PEXReactor", r)
	return r
}
...
func (r *PEXReactor) Receive(chID byte, src Peer, msgBytes []byte) {
	...
	case *pexRequestMessage:
		...
		if r.config.SeedMode && !src.IsOutbound() {
			...
			r.SendAddrs(src, GetSelectionWithBias(biasToSelectNewPeers))
			go func() {
...
func (r *PEXReactor) ReceiveAddrs(addrs []*p2p2.NetAddress, src Peer) error {
	id := string(src.ID())
	if !r.requestsSent.Has(id) {
	...
	for _, netAddr := range addrs {
		...
		err = AddAddress(netAddr, srcAddr)
		if err != nil {
			r.logErrAddrBook(err)
...
func (r *PEXReactor) SendAddrs(p Peer, netAddrs []*p2p2.NetAddress) {
...
func (r *PEXReactor) ensurePeers() {
	...
	toDial := make(map[p2p2.ID]*p2p2.NetAddress)
	// Try maxAttempts times to pick numToDial addresses to dial
	maxAttempts := numToDial * 3

	for i := 0; i < maxAttempts && len(toDial) < numToDial; i++ {
		try := PickAddress(newBias)
		if try == nil {
			continue
		}
...
func (r *PEXReactor) dialAttemptsInfo(addr *p2p2.NetAddress) (attempts int, lastDialed time.Time) {
...
func (r *PEXReactor) dialPeer(addr *p2p2.NetAddress) error {
	...
	if err != nil {
		if _, ok := err.(p2p2.ErrCurrentlyDialingOrExistingAddress); ok {
			return err
		}

		markAddrInBookBasedOnErr(addr, r.book, err)
		switch err.(type) {
		case p2p2.ErrSwitchAuthenticationFailure:
...
func (r *PEXReactor) checkSeeds() (numOnline int, netAddrs []*p2p2.NetAddress, err error) {
	lSeeds := len(r.config.Seeds)
	if lSeeds == 0 {
		return -1, nil, nil
	}
	netAddrs, errs := p2p2.NewNetAddressStrings(r.config.Seeds)
	numOnline = lSeeds - len(errs)
	for _, err := range errs {
		switch e := err.(type) {
		case p2p2.ErrNetAddressLookup:
...
func (r *PEXReactor) AttemptsToDial(addr *p2p2.NetAddress) int {
...
type crawlPeerInfo struct {
	Addr *p2p2.NetAddress `json:"addr"`
...
func (r *PEXReactor) crawlPeers(addrs []*p2p2.NetAddress) {
	now := time.Now()
...
func markAddrInBookBasedOnErr(addr *p2p2.NetAddress, book AddrBook, err error) {
