...
func (sw *Switch) AddReactor(name string, reactor Reactor) Reactor {
	for _, chDesc := range GetChannels() {
		chID := chDesc.ID
...
func (sw *Switch) StopPeerForError(peer Peer, reason interface{}) {
	...
	if IsPersistent() {
		var addr *NetAddress
		if IsOutbound() { // socket address for outbound peers
			addr = SocketAddr()
...
func (sw *Switch) filterPeer(p Peer) error {
	...
	for i := 0; i < cap(errc); i++ {
		...
		case err := <-errc:
			if err != nil {
				return ErrRejected{id: ID(), err: err, isFiltered: true}
			}
...
func (sw *Switch) addPeer(p Peer) error {
	...
	p.SetLogger(sw.Logger.With("peer", SocketAddr()))
