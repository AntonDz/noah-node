...
func (mem *CListMempool) CheckTx(tx types.Tx, cb func(*types2.Response)) (err error) {
	return mem.CheckTxWithInfo(tx, cb, TxInfo{SenderID: UnknownPeerID})
}

func (mem *CListMempool) CheckTxWithInfo(tx types.Tx, cb func(*types2.Response), txInfo TxInfo) (err error) {
	mem.proxyMtx.Lock()
	...
	reqRes := mem.proxyAppConn.CheckTxAsync(types2.RequestCheckTx{Tx: tx})
...
func (mem *CListMempool) globalCb(req *types2.Request, res *types2.Response) {
...
func (mem *CListMempool) reqResCb(tx []byte, peerID uint16, externalCb func(*types2.Response)) func(res *types2.Response) {
	return func(res *types2.Response) {
		if mem.recheckCursor != nil {
...
func (mem *CListMempool) resCbFirstTime(tx []byte, peerID uint16, res *types2.Response) {
	switch r := res.Value.(type) {
	case *types2.Response_CheckTx:
		var postCheckErr error
		if mem.postCheck != nil {
			postCheckErr = mem.postCheck(tx, r.CheckTx)
		}
		if (r.CheckTx.Code == types2.CodeTypeOK) && postCheckErr == nil {
			memTx := &mempoolTx{
				height:    mem.height,
...
func (mem *CListMempool) resCbRecheck(req *types2.Request, res *types2.Response) {
	switch r := res.Value.(type) {
	case *types2.Response_CheckTx:
...
func (mem *CListMempool) Update(
	height int64,
	txs types.Txs,
	deliverTxResponses []*types2.ResponseDeliverTx,
	preCheck PreCheckFunc,
	postCheck PostCheckFunc,
) error {
	// Set height
	...
	for i, tx := range txs {
		if deliverTxResponses[i].Code == types2.CodeTypeOK {
...
func (mem *CListMempool) recheckTxs() {
	...
	for e := mem.txs.Front(); e != nil; e = e.Next() {
		memTx := e.Value.(*mempoolTx)
		mem.proxyAppConn.CheckTxAsync(types2.RequestCheckTx{
			Tx:   memTx.tx,
			Type: types2.CheckTxType_Recheck,
		})
	}
