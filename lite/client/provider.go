...
import (
	"fmt"
	lite2 "github.com/noah-blockchain/noah-node/lite"
...
func NewProvider(chainID string, client SignStatusClient) lite2.Provider {
	return &provider{
...
func NewHTTPProvider(chainID, remote string) lite2.Provider {
...
func (p *provider) LatestFullCommit(chainID string, minHeight, maxHeight int64) (fc lite2.FullCommit, err error) {
...
func (p *provider) fillFullCommit(signedHeader types.SignedHeader) (fc lite2.FullCommit, err error) {

	// Get the validators.
	valset, err := p.getValidatorSet(signedHeader.ChainID, signedHeader.Height)
	if err != nil {
		return lite2.FullCommit{}, err
	}
	...
	return lite2.NewFullCommit(signedHeader, valset, nextValset), nil
}
