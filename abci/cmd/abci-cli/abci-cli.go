...
import (
	...
	"github.com/noah-blockchain/noah-node/abci/example/code"
	"github.com/noah-blockchain/noah-node/abci/example/counter"
	"github.com/noah-blockchain/noah-node/abci/example/kvstore"
	"github.com/noah-blockchain/noah-node/abci/server"
	server2 "github.com/noah-blockchain/noah-node/abci/tests/server"
	"github.com/noah-blockchain/noah-node/abci/types"
	"github.com/noah-blockchain/noah-node/abci/version"
	"io"
	"os"
	"strings"
...
var RootCmd = &cobra.Command{
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		if client == nil {
			var err error
			client, err = client2.NewClient(flagAddress, flagAbci, false)
			if err != nil {
				return err
			}
...
func cmdTest(cmd *cobra.Command, args []string) error {
	return compose(
		[]func() error{
			func() error { return server2.InitChain(client) },
			func() error { return server2.SetOption(client, "serial", "on") },
			func() error { return server2.Commit(client, nil) },
			func() error { return server2.DeliverTx(client, []byte("abc"), code.CodeTypeBadNonce, nil) },
			func() error { return server2.Commit(client, nil) },
			func() error { return server2.DeliverTx(client, []byte{0x00}, code.CodeTypeOK, nil) },
			func() error { return server2.Commit(client, []byte{0, 0, 0, 0, 0, 0, 0, 1}) },
			func() error { return server2.DeliverTx(client, []byte{0x00}, code.CodeTypeBadNonce, nil) },
			func() error { return server2.DeliverTx(client, []byte{0x01}, code.CodeTypeOK, nil) },
			func() error { return server2.DeliverTx(client, []byte{0x00, 0x02}, code.CodeTypeOK, nil) },
			func() error { return server2.DeliverTx(client, []byte{0x00, 0x03}, code.CodeTypeOK, nil) },
			func() error { return server2.DeliverTx(client, []byte{0x00, 0x00, 0x04}, code.CodeTypeOK, nil) },
			func() error {
				return server2.DeliverTx(client, []byte{0x00, 0x00, 0x06}, code.CodeTypeBadNonce, nil)
			},
			func() error { return server2.Commit(client, []byte{0, 0, 0, 0, 0, 0, 0, 5}) },
		})
}
