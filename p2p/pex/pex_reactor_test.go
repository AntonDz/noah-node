...
func TestPEXReactorAddRemovePeer(t *testing.T) {
	...
	size := book.Size()
	peer := p2p2.CreateRandomPeer(false)
	...
	outboundPeer := p2p2.CreateRandomPeer(true)
...
func TestPEXReactorRunning(t *testing.T) {
	...
	// create switches
	for i := 0; i < N; i++ {
		switches[i] = p2p2.MakeSwitch(cfg, i, "testing", "123.123.123", func(i int, sw *p2p2.Switch) *p2p2.Switch {
...
func TestPEXReactorReceive(t *testing.T) {
	...
	size := book.Size()
	addrs := []*p2p2.NetAddress{peer.SocketAddr()}
...
func TestPEXReactorRequestMessageAbuse(t *testing.T) {
	...
	p2p2.AddPeerToSwitchPeerSet(sw, peer)
...
func TestCheckSeeds(t *testing.T) {
	...
	// 2. create seed
	seed := testCreateSeed(dir, 1, []*p2p2.NetAddress{}, []*p2p2.NetAddress{})
...
func TestPEXReactorUsesSeedsIfNeeded(t *testing.T) {
	...
	// 1. create seed
	seed := testCreateSeed(dir, 0, []*p2p2.NetAddress{}, []*p2p2.NetAddress{})
	...
	assertPeersWithTimeout(t, []*p2p2.Switch{peer}, 10*time.Millisecond, 3*time.Second, 1)
}
...
func TestConnectionSpeedForPeerReceivedFromSeed(t *testing.T) {
	...
	seed := testCreateSeed(dir, 2, []*p2p2.NetAddress{peerAddr}, []*p2p2.NetAddress{peerAddr})
	require.Nil(t, seed.Start())
	defer seed.Stop()
	...
	assertPeersWithTimeout(t, []*p2p2.Switch{secondPeer}, 10*time.Millisecond, 3*time.Second, 1)

	// 5. check that the second peer connects to the first peer immediately
	assertPeersWithTimeout(t, []*p2p2.Switch{secondPeer}, 10*time.Millisecond, 1*time.Second, 2)
}
...
func TestPEXReactorSeedMode(t *testing.T) {
	...
	pexR.crawlPeers([]*p2p2.NetAddress{peerSwitch.NetAddress()})
...
func TestPEXReactorDialsPeerUpToMaxAttemptsInSeedMode(t *testing.T) {
	...
	pexR.crawlPeers([]*p2p2.NetAddress{addr})
...
func testCreatePeerWithConfig(dir string, id int, config *PEXReactorConfig) *p2p2.Switch {
	peer := p2p2.MakeSwitch(
		cfg,
		id,
		"127.0.0.1",
		"123.123.123",
		func(i int, sw *p2p2.Switch) *p2p2.Switch {
...
func testCreateDefaultPeer(dir string, id int) *p2p2.Switch {
...
func testCreateSeed(dir string, id int, knownAddrs, srcAddrs []*p2p2.NetAddress) *p2p2.Switch {
	seed := p2p2.MakeSwitch(
		cfg,
		id,
		"127.0.0.1",
		"123.123.123",
...
func testCreatePeerWithSeed(dir string, id int, seed *p2p2.Switch) *p2p2.Switch {
...
func createSwitchAndAddReactors(reactors ...p2p2.Reactor) *p2p2.Switch {
	sw := p2p2.MakeSwitch(cfg, 0, "127.0.0.1", "123.123.123", func(i int, sw *p2p2.Switch) *p2p2.Switch { return sw })
	sw.SetLogger(log.TestingLogger())
