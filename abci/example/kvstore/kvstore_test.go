...
func makeSocketClientServer(app types.Application, name string) (client.Client, cmn.Service, error) {
	// Start the listener
	socket := fmt.Sprintf("unix://%s.sock", name)
	logger := log.TestingLogger()

	server := server3.NewSocketServer(socket, app)
	...
	client := client.NewSocketClient(socket, false)
...
func makeGRPCClientServer(app types.Application, name string) (client.Client, cmn.Service, error) {
	// Start the listener
	socket := fmt.Sprintf("unix://%s.sock", name)
	logger := log.TestingLogger()

	gapp := types.NewGRPCApplication(app)
	server := server3.NewGRPCServer(socket, gapp)
	server.SetLogger(logger.With("module", "abci-server"))
	if err := server.Start(); err != nil {
		return nil, nil, err
	}

	client := client.NewGRPCClient(socket, true)
	client.SetLogger(logger.With("module", "abci-client"))
	if err := client.Start(); err != nil {
		server.Stop()
		return nil, nil, err
...
func runClientTests(t *testing.T, client client.Client) {
	// run some tests....
	key := "abc"
	value := key
	tx := []byte(key)
...
func testClient(t *testing.T, app client.Client, tx []byte, key, value string) {
