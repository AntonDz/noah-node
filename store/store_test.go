...
func makeBlock(height int64, state state2.State, lastCommit *types.Commit) *types.Block {
	block, _ := state.MakeBlock(height, makeTxs(height), lastCommit, nil, state.Validators.GetProposer().Address)
	return block
}

func makeStateAndBlockStore(logger log.Logger) (state2.State, *BlockStore, cleanupFunc) {
	config := cfg.ResetTestRoot("blockchain_reactor_test")
	// blockDB := dbm.NewDebugDB("blockDB", dbm.NewMemDB())
	// stateDB := dbm.NewDebugDB("stateDB", dbm.NewMemDB())
	blockDB := dbm.NewMemDB()
	stateDB := dbm.NewMemDB()
	state, err := state2.LoadStateFromDBOrGenesisFile(stateDB, config.GenesisFile())
	if err != nil {
...
func TestMain(m *testing.M) {
	...
	seenCommit1 = makeTestCommit(10, time2.Now())
	code := m.Run()
	cleanup()
	os.Exit(code)
}
...
func TestBlockStoreSaveLoadBlock(t *testing.T) {
	...
	seenCommit := makeTestCommit(10, time2.Now())
	...
	commitAtH10 := makeTestCommit(10, time2.Now())
	tuples := []struct {
		block      *types.Block
		parts      *types.PartSet
