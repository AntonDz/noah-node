...
import (
	"bytes"
	"fmt"
	types2 "github.com/noah-blockchain/noah-node/abci/types"
	"math"
	"math/big"
	"os"
	"testing"
...
func TestABCIResponsesSaveLoad1(t *testing.T) {
	...
	abciResponses.DeliverTx[0] = &types2.ResponseDeliverTx{Data: []byte("foo"), Events: nil}
	abciResponses.DeliverTx[1] = &types2.ResponseDeliverTx{Data: []byte("bar"), Log: "ok", Events: nil}
	abciResponses.EndBlock = &types2.ResponseEndBlock{ValidatorUpdates: []types2.ValidatorUpdate{
...
func TestABCIResponsesSaveLoad2(t *testing.T) {
	...
	cases := [...]struct {
		...
		added    []*types2.ResponseDeliverTx
	...
	for i, tc := range cases {
		...
		responses := &ABCIResponses{
			DeliverTx: tc.added,
			EndBlock:  &types2.ResponseEndBlock{},
		}
...
func TestValidatorSimpleSaveLoad(t *testing.T) {
	...
	_, err := LoadValidators(stateDB, 0)
...
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	...
	abciResponses := &ABCIResponses{
		EndBlock: &types2.ResponseEndBlock{ValidatorUpdates: nil},
	}
	...
	updateAddVal := types2.ValidatorUpdate{PubKey: types.TM2PB.PubKey(val2PubKey), Power: val2VotingPower}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates([]types2.ValidatorUpdate{updateAddVal})
	assert.NoError(t, err)
	...
	updateVal := types2.ValidatorUpdate{PubKey: types.TM2PB.PubKey(val2PubKey), Power: updatedVotingPowVal2}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates([]types2.ValidatorUpdate{updateVal})
	assert.NoError(t, err)
...
func TestProposerPriorityProposerAlternates(t *testing.T) {
	...
	val2PubKey := ed25519.GenPrivKey().PubKey()
	updateAddVal := types2.ValidatorUpdate{PubKey: types.TM2PB.PubKey(val2PubKey), Power: val1VotingPower}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates([]types2.ValidatorUpdate{updateAddVal})
	assert.NoError(t, err)
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	firstAddedVal := types2.ValidatorUpdate{PubKey: types.TM2PB.PubKey(firstAddedValPubKey), Power: firstAddedValVotingPower}
	validatorUpdates, err := types.PB2TM.ValidatorUpdates([]types2.ValidatorUpdate{firstAddedVal})
	assert.NoError(t, err)
	abciResponses := &ABCIResponses{
		EndBlock: &types2.ResponseEndBlock{ValidatorUpdates: []types2.ValidatorUpdate{firstAddedVal}},
	}
	block := makeBlock(oldState, oldState.LastBlockHeight+1)
	blockID := types.BlockID{Hash: block.Hash(), PartsHeader: block.MakePartSet(testPartSize).Header()}
	updatedState, err := UpdateState(oldState, blockID, &block.Header, abciResponses, validatorUpdates)
	...
	for i := 0; i < 10; i++ {
		...
		addedVal := types2.ValidatorUpdate{PubKey: types.TM2PB.PubKey(addedPubKey), Power: firstAddedValVotingPower}
		validatorUpdates, err := types.PB2TM.ValidatorUpdates([]types2.ValidatorUpdate{addedVal})
		assert.NoError(t, err)

		abciResponses := &ABCIResponses{
			EndBlock: &types2.ResponseEndBlock{ValidatorUpdates: []types2.ValidatorUpdate{addedVal}},
		}
		block := makeBlock(oldState, oldState.LastBlockHeight+1)
		blockID := types.BlockID{Hash: block.Hash(), PartsHeader: block.MakePartSet(testPartSize).Header()}
	...
	removeGenesisVal := types2.ValidatorUpdate{PubKey: types.TM2PB.PubKey(genesisPubKey), Power: 0}
	abciResponses = &ABCIResponses{
		EndBlock: &types2.ResponseEndBlock{ValidatorUpdates: []types2.ValidatorUpdate{removeGenesisVal}},
	}
	block = makeBlock(oldState, oldState.LastBlockHeight+1)
	blockID = types.BlockID{Hash: block.Hash(), PartsHeader: block.MakePartSet(testPartSize).Header()}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates(abciResponses.EndBlock.ValidatorUpdates)
...
func TestApplyUpdates(t *testing.T) {
	...
	cases := [...]struct {
		...
		updates  types2.ConsensusParams
		expected types.ConsensusParams
	}{
		0: {initParams, types2.ConsensusParams{}, initParams},
		1: {initParams, types2.ConsensusParams{}, initParams},
		2: {initParams,
		...
		3: {initParams,
			types2.ConsensusParams{
				Evidence: &types2.EvidenceParams{
					MaxAge: 66,
				},
			},
