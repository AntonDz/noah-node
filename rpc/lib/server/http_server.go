...
func WriteRPCResponseHTTP(w http.ResponseWriter, res types2.RPCResponse) {
	jsonBytes, err := json.Marshal(res)
	if err != nil {
		panic(err)
	}
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		defer func() {
			...
			if e := recover(); e != nil {
				...
				// If RPCResponse
				if res, ok := e.(types2.RPCResponse); ok {
					...
					WriteRPCResponseHTTPError(rww, http.StatusInternalServerError, types2.RPCInternalError(types2.JSONRPCStringID(""), e.(error)))
				}
			}
