...
type Mempool interface {
	...
	CheckTx(tx types.Tx, callback func(*types2.Response)) error
	...
	CheckTxWithInfo(tx types.Tx, callback func(*types2.Response), txInfo TxInfo) error
	...
	Update(blockHeight int64, blockTxs types.Txs, deliverTxResponses []*types2.ResponseDeliverTx, newPreFn PreCheckFunc, newPostFn PostCheckFunc) error
...
type PostCheckFunc func(types.Tx, *types2.ResponseCheckTx) error
...
func PostCheckMaxGas(maxGas int64) PostCheckFunc {
	return func(tx types.Tx, res *types2.ResponseCheckTx) error {
		if maxGas == -1 {
			return nil
		}
		if res.GasWanted < 0 {
