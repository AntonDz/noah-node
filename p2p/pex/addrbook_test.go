...
func randIPv4Address(t *testing.T) *p2p2.NetAddress {
	for {
		...
		id := p2p2.ID(hex.EncodeToString(cmn.RandBytes(p2p2.IDByteLength)))
		idAddr := p2p2.IDAddressString(id, fmt.Sprintf("%v:%v", ip, port))
		addr, err := p2p2.NewNetAddressString(idAddr)
...
func TestAddrBookGetSelection(t *testing.T) {
	...
	addrs := make(map[string]*p2p2.NetAddress)
	selection := book.GetSelection()
...
func testCreatePrivateAddrs(t *testing.T, numAddrs int) ([]*p2p2.NetAddress, []string) {
	addrs := make([]*p2p2.NetAddress, numAddrs)
...
func assertMOldAndNNewAddrsInSelection(t *testing.T, m, n int, addrs []*p2p2.NetAddress, book *addrBook) {
	nOld, nNew := countOldAndNewAddrsInSelection(addrs, book)
...
func countOldAndNewAddrsInSelection(addrs []*p2p2.NetAddress, book *addrBook) (nOld, nNew int) {
	for _, addr := range addrs {
		if book.IsGood(addr) {
			nOld++
		} else {
...
func analyseSelectionLayout(book *addrBook, addrs []*p2p2.NetAddress) (seqLens, seqTypes []int, err error) {
