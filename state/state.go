...
func MedianTime(commit *types.Commit, validators *types.ValidatorSet) time.Time {
	...
	weightedTimes := make([]*time2.WeightedTime, len(commit.Precommits))
	...
	for i, vote := range commit.Precommits {
		if vote != nil {
			...
			weightedTimes[i] = time2.NewWeightedTime(vote.Timestamp, validator.VotingPower)
		}
	}

	return time2.WeightedMedian(weightedTimes, totalVotingPower)
}
