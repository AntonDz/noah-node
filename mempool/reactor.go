...
// peer.
func (ids *mempoolIDs) ReserveForPeer(peer p2p2.Peer) {
	ids.mtx.Lock()
...
func (ids *mempoolIDs) Reclaim(peer p2p2.Peer) {
	ids.mtx.Lock()
	defer ids.mtx.Unlock()
...
func (ids *mempoolIDs) GetForPeer(peer p2p2.Peer) uint16 {
	ids.mtx.RLock()
...
func newMempoolIDs() *mempoolIDs {
	return &mempoolIDs{
		peerMap:   make(map[p2p2.ID]uint16),
...
func NewReactor(config *cfg.MempoolConfig, mempool *CListMempool) *Reactor {
	...
	memR.BaseReactor = *p2p2.NewBaseReactor("Reactor", memR)
	return memR
}
...
func (memR *Reactor) GetChannels() []*p2p2.ChannelDescriptor {
	return []*p2p2.ChannelDescriptor{
		{
			ID:       MempoolChannel,
			Priority: 5,
		},
...
func (memR *Reactor) AddPeer(peer p2p2.Peer) {
	memR.ids.ReserveForPeer(peer)
	go memR.broadcastTxRoutine(peer)
}

// RemovePeer implements Reactor.
func (memR *Reactor) RemovePeer(peer p2p2.Peer, reason interface{}) {
...
func (memR *Reactor) Receive(chID byte, src p2p2.Peer, msgBytes []byte) {
...
func (memR *Reactor) broadcastTxRoutine(peer p2p2.Peer) {
	if !memR.config.Broadcast {
		return
	}
