...
import (
	...
	"encoding/json"
	"fmt"
	types2 "github.com/noah-blockchain/noah-node/rpc/lib/types"
	"io/ioutil"
	"math/big"
	"net/http"
	"reflect"
...
func makeJSONRPCHandler(funcMap map[string]*RPCFunc, cdc *amino.Codec, logger log.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			WriteRPCResponseHTTP(w, types2.RPCInvalidRequestError(types2.JSONRPCStringID(""), errors.Wrap(err, "Error reading request body")))
			return
		}
		...
		var request types2.RPCRequest
		err = json.Unmarshal(b, &request)
		if err != nil {
			WriteRPCResponseHTTP(w, types2.RPCParseError(types2.JSONRPCStringID(""), errors.Wrap(err, "Error unmarshalling request")))
			return
		}
		// A Notification is a Request object without an "id" member.
		// The Server MUST NOT reply to a Notification, including those that are within a batch request.
		if request.ID == types2.JSONRPCStringID("") {
			logger.Debug("HTTPJSONRPC received a notification, skipping... (please send a non-empty ID if you want to call a method)")
			return
		}
		if len(r.URL.Path) > 1 {
			WriteRPCResponseHTTP(w, types2.RPCInvalidRequestError(request.ID, errors.Errorf("Path %s is invalid", r.URL.Path)))
			return
		}
		rpcFunc := funcMap[request.Method]
		if rpcFunc == nil || rpcFunc.ws {
			WriteRPCResponseHTTP(w, types2.RPCMethodNotFoundError(request.ID))
			return
		}
		...
		if len(request.Params) > 0 {
			...
			if err != nil {
				WriteRPCResponseHTTP(w, types2.RPCInvalidParamsError(request.ID, errors.Wrap(err, "Error converting json params to arguments")))
				return
			}
		}
		...
		if err != nil {
			WriteRPCResponseHTTP(w, types2.RPCInternalError(request.ID, err))
			return
		}
		WriteRPCResponseHTTP(w, types2.NewRPCSuccessResponse(cdc, request.ID, result))
	}
}
...
func jsonParamsToArgsWS(rpcFunc *RPCFunc, cdc *amino.Codec, params json.RawMessage, wsCtx types2.WSRPCContext) ([]reflect.Value, error) {
...
func makeHTTPHandler(rpcFunc *RPCFunc, cdc *amino.Codec, logger log.Logger) func(http.ResponseWriter, *http.Request) {
	...
	if rpcFunc.ws {
		return func(w http.ResponseWriter, r *http.Request) {
			WriteRPCResponseHTTP(w, types2.RPCMethodNotFoundError(types2.JSONRPCStringID("")))
		}
	}
	// All other endpoints
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			WriteRPCResponseHTTP(w, types2.RPCInvalidParamsError(types2.JSONRPCStringID(""), errors.Wrap(err, "Error converting http params to arguments")))
			return
		}
		returns := rpcFunc.f.Call(args)
		...
		result, err := unreflectResult(returns)
		if err != nil {
			WriteRPCResponseHTTP(w, types2.RPCInternalError(types2.JSONRPCStringID(""), err))
			return
		}
		WriteRPCResponseHTTP(w, types2.NewRPCSuccessResponse(cdc, types2.JSONRPCStringID(""), result))
	}
}
...
type wsConnection struct {
	...
	writeChan  chan types2.RPCResponse
	...
	eventSub types2.EventSubscriber
}
...
// used.
func EventSubscriber(eventSub types2.EventSubscriber) func(*wsConnection) {
...
func (wsc *wsConnection) OnStart() error {
	wsc.writeChan = make(chan types2.RPCResponse, wsc.writeChanCapacity)
...
func (wsc *wsConnection) GetEventSubscriber() types2.EventSubscriber {
	return wsc.eventSub
}

// WriteRPCResponse pushes a response to the writeChan, and blocks until it is accepted.
// It implements WSRPCConnection. It is Goroutine-safe.
func (wsc *wsConnection) WriteRPCResponse(resp types2.RPCResponse) {
	select {
	case <-wsc.Quit():
		return
...
// It implements WSRPCConnection. It is Goroutine-safe
func (wsc *wsConnection) TryWriteRPCResponse(resp types2.RPCResponse) bool {
	select {
	case <-wsc.Quit():
		return false
	case wsc.writeChan <- resp:
...
func (wsc *wsConnection) readRoutine() {
	defer func() {
		if r := recover(); r != nil {
			...
			wsc.WriteRPCResponse(types2.RPCInternalError(types2.JSONRPCStringID("unknown"), err))
			go wsc.readRoutine()
		} else {
	...
	for {
		...
		default:
			...
			if err != nil {
				wsc.WriteRPCResponse(types2.RPCParseError(types2.JSONRPCStringID(""), errors.Wrap(err, "Error unmarshaling request")))
				continue
			}
			...
			if rpcFunc == nil {
				wsc.WriteRPCResponse(types2.RPCMethodNotFoundError(request.ID))
				continue
			}
			var args []reflect.Value
			if rpcFunc.ws {
				wsCtx := types2.WSRPCContext{Request: request, WSRPCConnection: wsc}
				if len(request.Params) > 0 {
			...
			if err != nil {
				wsc.WriteRPCResponse(types2.RPCInternalError(request.ID, errors.Wrap(err, "Error converting json params to arguments")))
				continue
			}
			returns := rpcFunc.f.Call(args)
			...
			result, err := unreflectResult(returns)
			if err != nil {
				wsc.WriteRPCResponse(types2.RPCInternalError(request.ID, err))
				continue
			}

			wsc.WriteRPCResponse(types2.NewRPCSuccessResponse(wsc.cdc, request.ID, result))
		}
	}
}
...
func unreflectResult(returns []reflect.Value) (interface{}, error) {
	...
	if errV.Interface() != nil {
		if txErr, ok := errV.Interface().(types2.TxError); ok {
			return nil, txErr
		}
		if rpcErr, ok := errV.Interface().(types2.RPCError); ok {
			return nil, rpcErr
		}
