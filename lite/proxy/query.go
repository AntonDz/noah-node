...
func GetWithProofOptions(prt *merkle.ProofRuntime, path string, key []byte, opts rpcclient.ABCIQueryOptions,
	node rpcclient.Client, cert lite2.Verifier) (
	*ctypes.ResultABCIQuery, error) {
	...
	if len(resp.Key) == 0 || resp.Proof == nil {
		return nil, errors3.ErrEmptyTree()
	}
	if resp.Height == 0 {
...
func GetCertifiedCommit(h int64, client rpcclient.Client, cert lite2.Verifier) (types.SignedHeader, error) {
