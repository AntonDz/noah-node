...
import (
	...
	"github.com/noah-blockchain/noah-node/p2p/pex"
	"github.com/noah-blockchain/noah-node/state"
	"github.com/noah-blockchain/noah-node/state/txindex"
	"github.com/noah-blockchain/noah-node/state/txindex/kv"
	"github.com/noah-blockchain/noah-node/state/txindex/null"
	...
	"github.com/noah-blockchain/noah-node/store"
...
type MetricsProvider func(chainID string) (*cs.Metrics, *p2p.Metrics, *mempl.Metrics, *state.Metrics)

// DefaultMetricsProvider returns Metrics build using Prometheus client library
// if Prometheus is enabled. Otherwise, it returns no-op Metrics.
func DefaultMetricsProvider(config *cfg.InstrumentationConfig) MetricsProvider {
	return func(chainID string) (*cs.Metrics, *p2p.Metrics, *mempl.Metrics, *state.Metrics) {
		...
		return cs.NopMetrics(), p2p.NopMetrics(), mempl.NopMetrics(), state.NopMetrics()
	}
}
...
func doHandshake(stateDB dbm.DB, state state.State, blockStore state.BlockStore,
...
func logNodeStartupInfo(state state.State, pubKey crypto.PubKey, logger, consensusLogger log.Logger) {
...
func onlyValidatorIsUs(state state.State, privVal types.PrivValidator) bool {
...
func createMempoolAndMempoolReactor(config *cfg.Config, proxyApp proxy.AppConns,
	state state.State, memplMetrics *mempl.Metrics, logger log.Logger) (*mempl.Reactor, *mempl.CListMempool) {
	...
	mempool := mempl.NewCListMempool(
		...
		mempl.WithPreCheck(state.TxPreCheck(state)),
		mempl.WithPostCheck(state.TxPostCheck(state)),
	)
	...
	if config.FilterPeers {
		connFilters = append(
			...
			func(_ p2p.ConnSet, c net.Conn, _ []net.IP) error {
				res, err := proxyApp.Query().QuerySync(types2.RequestQuery{
...
func NewNode(config *cfg.Config,
	...
	blockExec := state.NewBlockExecutor(
		stateDB,
		logger.With("module", "state"),
		proxyApp.Consensus(),
		mempool,
		evidencePool,
		state.BlockExecutorWithMetrics(smMetrics),
	)
...
func LoadStateFromDBOrGenesisDocProvider(stateDB dbm.DB, genesisDocProvider GenesisDocProvider) (state.State, *types.GenesisDoc, error) {
	// Get genesis doc
	...
	state, err := state.LoadStateFromDBOrGenesisDoc(stateDB, genDoc)
	if err != nil {
		return state.State{}, nil, err
	}
