...
import (
	"bytes"
	"fmt"
	"github.com/noah-blockchain/noah-node/lite/errors"
	"sync"
...
func (dv *DynamicVerifier) Verify(shdr types.SignedHeader) error {
	...
	// just return nil.
	trustedFCSameHeight, err := LatestFullCommit(dv.chainID, shdr.Height, shdr.Height)
	if err == nil {
	...
	} else if !errors.IsErrCommitNotFound(err) {
	...
	trustedFC, err := LatestFullCommit(dv.chainID, 1, prevHeight)
	if err != nil {
		return err
	}
	...
	if trustedFC.Height() == prevHeight {
		...
		if !bytes.Equal(
			...
			return errors.ErrUnexpectedValidators(
				trustedFC.NextValidators.Hash(),
				shdr.Header.ValidatorsHash)
		}
	} else {
	...
	nextValset, err := ValidatorSet(dv.chainID, shdr.Height+1)
	if errors.IsErrUnknownValidators(err) {
		// Ignore this error.
		return nil
	} else if err != nil {
		return err
...
func (dv *DynamicVerifier) verifyAndSave(trustedFC, sourceFC FullCommit) error {
	...
	return SaveFullCommit(sourceFC)
}
...
func (dv *DynamicVerifier) updateToHeight(h int64) (FullCommit, error) {
	...
	sourceFC, err := LatestFullCommit(dv.chainID, h, h)
	if err != nil {
	...
	if sourceFC.Height() != h {
		return FullCommit{}, errors.ErrCommitNotFound()
	}
...
FOR_LOOP:
	for {
		...
		trustedFC, err := LatestFullCommit(dv.chainID, 1, h)
		if err != nil {
			return FullCommit{}, err
		}
...
func (dv *DynamicVerifier) LastTrustedHeight() int64 {
	fc, err := LatestFullCommit(dv.chainID, 1, 1<<63-1)
	if err != nil {
		panic("should not happen")
	}
	return fc.Height()
