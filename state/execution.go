...
func (blockExec *BlockExecutor) CreateProposalBlock(
	...
	evidence := PendingEvidence(maxNumEvidence)
...
func execBlockOnProxyApp(
	...
	proxyCb := func(req *types2.Request, res *types2.Response) {
		if r, ok := res.Value.(*types2.Response_DeliverTx); ok {
			// TODO: make use of res.Log
			// TODO: make use of this info
			// Blocks may include invalid txs.
			txRes := r.DeliverTx
			if txRes.Code == types2.CodeTypeOK {
				validTxs++
			} else {
	...
	// Begin block
	var err error
	abciResponses.BeginBlock, err = proxyAppConn.BeginBlockSync(types2.RequestBeginBlock{
		Hash:                block.Hash(),
	...
	for _, tx := range block.Txs {
		proxyAppConn.DeliverTxAsync(types2.RequestDeliverTx{Tx: tx})
	...
	// End block.
	abciResponses.EndBlock, err = proxyAppConn.EndBlockSync(types2.RequestEndBlock{Height: block.Height})
	if err != nil {
...
func getBeginBlockValidatorInfo(block *types.Block, stateDB dbm.DB) (types2.LastCommitInfo, []types2.Evidence) {
	voteInfos := make([]types2.VoteInfo, block.LastCommit.Size())
	byzVals := make([]types2.Evidence, len(block.Evidence.Evidence))
	...
	commitInfo := types2.LastCommitInfo{
...
func validateValidatorUpdates(abciUpdates []types2.ValidatorUpdate,
