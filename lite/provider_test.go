...
import (
	"errors"
	errors3 "github.com/noah-blockchain/noah-node/lite/errors"
	"testing"
...
func (missingProvider) LatestFullCommit(chainID string, minHeight, maxHeight int64) (FullCommit, error) {
	return FullCommit{}, errors3.ErrCommitNotFound()
}
...
func checkProvider(t *testing.T, p PersistentProvider, chainID, app string) {
	...
	fc, err := LatestFullCommit(chainID, 1, 1<<63-1)
	require.NotNil(err)
	assert.True(errors3.IsErrCommitNotFound(err))
	...
	for _, fc := range fcz {
		...
		fc2, err := LatestFullCommit(chainID, fc.Height(), fc.Height())
		assert.Nil(err)
	...
	fc, err = LatestFullCommit(chainID, 1, 5000)
	if assert.Nil(err) {
	...
	fc, err = LatestFullCommit(chainID, 1, 47)
	if assert.Nil(err) {
...
func checkLatestFullCommit(t *testing.T, p PersistentProvider, chainID string, ask, expect int64) {
	fc, err := LatestFullCommit(chainID, 1, ask)
	require.Nil(t, err)
